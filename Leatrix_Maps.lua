	...
	----------------------------------------------------------------------
	-- 	Leatrix Maps 1.13.08 (9th August 2019, www.leatrix.com)
	----------------------------------------------------------------------
	...
	function LeaMapsLC:MainFunc()
		...
		do
			...
			-- Create fade frame
			local fadeFrame = CreateFrame("FRAME", nil, UIParent)
			LeaMapsLC["fadeFrame"] = fadeFrame
			fadeFrame:ClearAllPoints()
			fadeFrame:SetPoint("CENTER", UIParent, "CENTER", 0, 0)
			fadeFrame:SetSize(370, 340)
			fadeFrame:Hide()
			fadeFrame:SetFrameStrata("FULLSCREEN_DIALOG")
			fadeFrame:SetFrameLevel(20)
			fadeFrame:SetClampedToScreen(true)
			fadeFrame:EnableMouse(true)
			fadeFrame:SetMovable(true)
			fadeFrame:RegisterForDrag("LeftButton")
			fadeFrame:SetScript("OnDragStart", fadeFrame.StartMoving)
			fadeFrame:SetScript("OnDragStop", function()
				fadeFrame:StopMovingOrSizing()
				fadeFrame:SetUserPlaced(false)
				LeaMapsLC["MainPanelA"], void, LeaMapsLC["MainPanelR"], LeaMapsLC["MainPanelX"], LeaMapsLC["MainPanelY"] = fadeFrame:GetPoint()
			end)

			-- Set panel attributes when shown
			fadeFrame:SetScript("OnShow", function()
				fadeFrame:ClearAllPoints()
				fadeFrame:SetPoint(LeaMapsLC["MainPanelA"], UIParent, LeaMapsLC["MainPanelR"], LeaMapsLC["MainPanelX"], LeaMapsLC["MainPanelY"])
			end)

			-- Make it a system frame
			_G["LeaMapsGlobalFadePanel"] = fadeFrame
			table.insert(UISpecialFrames, "LeaMapsGlobalFadePanel")

			-- Add background color
			fadeFrame.t = fadeFrame:CreateTexture(nil, "BACKGROUND")
			fadeFrame.t:SetAllPoints()
			fadeFrame.t:SetColorTexture(0.05, 0.05, 0.05, 0.9)

			-- Add textures
			fadeFrame.mt = fadeFrame:CreateTexture(nil, "BORDER")
			fadeFrame.mt:SetTexture("Interface\\ACHIEVEMENTFRAME\\UI-GuildAchievement-Parchment-Horizontal-Desaturated.png")
			fadeFrame.mt:SetSize(370, 293)
			fadeFrame.mt:SetPoint("TOPRIGHT")
			fadeFrame.mt:SetVertexColor(0.7, 0.7, 0.7, 0.7)

			fadeFrame.ft = fadeFrame:CreateTexture(nil, "BORDER")
			fadeFrame.ft:SetTexture("Interface\\ACHIEVEMENTFRAME\\UI-GuildAchievement-Parchment-Horizontal-Desaturated.png")
			fadeFrame.ft:SetSize(370, 48)
			fadeFrame.ft:SetPoint("BOTTOM")
			fadeFrame.ft:SetVertexColor(0.5, 0.5, 0.5, 1.0)

			-- Add close Button
			local CloseB = CreateFrame("Button", nil, fadeFrame, "UIPanelCloseButton") 
			CloseB:SetSize(30, 30)
			CloseB:SetPoint("TOPRIGHT", 0, 0)

			-- Add content
			fadeFrame.mt = fadeFrame:CreateFontString(nil, 'ARTWORK', 'GameFontNormalLarge')
			fadeFrame.mt:SetPoint('TOPLEFT', 16, -16)
			fadeFrame.mt:SetText(L["Fade Map"])

			-- Add description
			fadeFrame.v = fadeFrame:CreateFontString(nil, 'ARTWORK', 'GameFontHighlightSmall')
			fadeFrame.v:SetHeight(32);
			fadeFrame.v:SetPoint('TOPLEFT', fadeFrame.mt, 'BOTTOMLEFT', 0, -8)
			fadeFrame.v:SetPoint('RIGHT', fadeFrame, -32, 0)
			fadeFrame.v:SetJustifyH('LEFT'); fadeFrame.v:SetJustifyV('TOP');
			fadeFrame.v:SetText(L["Configuration Panel"])
			...
			local fadeConfirmBtn = LeaMapsLC:CreateButton("fadeConfirmBtn", fadeFrame, "Back to Main Menu", "BOTTOMRIGHT", -16, 10, 25)
			fadeConfirmBtn:HookScript("OnClick", function()
				fadeFrame:Hide()
				LeaMapsLC["PageF"]:Show()
			end)

			-- Reset button click
			local fadeResetBtn = LeaMapsLC:CreateButton("fadeResetBtn", fadeFrame, "Reset", "BOTTOMLEFT", 16, 10, 25)
			fadeResetBtn:HookScript("OnClick", function()
		...
		do
			...
			-- Function to set blackout
			local function SetBlackout()
				if LeaMapsLC["NoBlackout"] == "On" then
			...
			-- Set blackout when option is clicked and on startup
			LeaMapsCB["NoBlackout"]:HookScript("OnClick", SetBlackout)
			SetBlackout()
	...
	local function SlashFunc(str)
		...
		if str and str ~= "" then
			...
			-- Hide configuraiton panels if they are showing and do nothing
			if LeaMapsLC["fadeFrame"]:IsShown() then LeaMapsLC["fadeFrame"]:Hide() return end
			if LeaMapsLC["tintFrame"]:IsShown() then LeaMapsLC["tintFrame"]:Hide() return end
	...
	eFrame:SetScript("OnEvent", function(self, event, arg1)
		...
		if event == "ADDON_LOADED" and arg1 == "Leatrix_Maps" then
			...
			LeaMapsLC:LoadVarChk("NoBlackout", "On")					-- Hide blackout frame
		...
		elseif event == "PLAYER_LOGOUT" and not LeaMapsLC["NoSaveSettings"] then
			...
			LeaMapsDB["NoBlackout"] = LeaMapsLC["NoBlackout"]
	...
	LeaMapsLC:MakeCB(PageF, "NoBlackout", "Hide map blackout frame", 16, -132, false)
	LeaMapsLC:MakeCB(PageF, "FadeMap", "Fade map when moving", 16, -152, false)
