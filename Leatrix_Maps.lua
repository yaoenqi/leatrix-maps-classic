	...
	----------------------------------------------------------------------
	-- 	Leatrix Maps 1.13.17 (14th August 2019, www.leatrix.com)
	----------------------------------------------------------------------
			...
			hooksecurefunc("ToggleWorldMap", function()
				if LeaMapsLC["RememberZoom"] == "On" then
					if WorldMapFrame:IsShown() then
						-- Map has been shown so set scale and pan settings if map ID has not changed
						if WorldMapFrame.mapID == lastMapID then
							WorldMapFrame.ScrollContainer:InstantPanAndZoom(lastZoomLevel, 0.5, 0.5)
							WorldMapFrame.ScrollContainer:SetNormalizedHorizontalScroll(lastHorizontal)
							WorldMapFrame.ScrollContainer:SetNormalizedVerticalScroll(lastVertical)
						end
					else
						-- Map has been hidden so save scale and pan settings
	...
	resetMapPosBtn:HookScript("OnClick", function()
		LeaMapsDB["MapPosA"], LeaMapsDB["MapPosR"], LeaMapsDB["MapPosX"], LeaMapsDB["MapPosY"] = "CENTER", "CENTER", 0, 0
		WorldMapFrame:ClearAllPoints()
		WorldMapFrame:SetPoint(LeaMapsDB["MapPosA"], UIParent, LeaMapsDB["MapPosR"], LeaMapsDB["MapPosX"], LeaMapsDB["MapPosY"])
	end)
